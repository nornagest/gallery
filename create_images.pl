...
sub create_dirs {
  ...
  for(my $i=2; $i <= $#parts; $i++) {
      ...
      mkdir $preview_path, $default_permissions unless -e $preview_path && -d $preview_path ;
      mkdir $thumb_path, $default_permissions unless -e $thumb_path && -d $thumb_path;
  }
}
...
sub process_file {
  ...
  my ($width, $height, $size, $format) = $image->Ping($file) or carp "Can't ping $file $1\n";
